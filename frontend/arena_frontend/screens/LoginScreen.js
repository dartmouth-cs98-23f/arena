...
function LoginScreen({ navigation }) {
  const handleLogin = async () => {
    const result = await WebBrowser.openAuthSessionAsync(
      `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=988417806604-ggnkhrhere0el8b4r3ehko3ncmt8181r.apps.googleusercontent.com&redirect_uri=http://arena-backend.fly.dev/auth&scope=https://www.googleapis.com/auth/userinfo.email%20https://www.googleapis.com/auth/userinfo.profile&access_type=offline&state=1234_purpleGoogle&prompt=consent`,
      "http://arena-backend.fly.dev/auth"
    )
